...
inline void frame_rendering_setup(const Camera& camera,
...
public:
    void render(const Geometry& geometry, const Shader& shader,
        ...
        auto& gpu_buffer = load_gpu_buffer(geometry, shader);
...
private:
    ...
    GpuBuffer& load_gpu_buffer(const Geometry& geometry,
    ...
    mutable unordered_map<string, GpuBuffer> hashed_buffers;
};
